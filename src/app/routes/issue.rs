        ...
        {move || match issue.get(){
            ...
            Some(Some((issue, merged_pull_requests, opened_pull_requests, opened_issues))) => {
                view!{
                    ...
                    <div
                        class=r#"prose prose-slate mt-14 [&>h2:nth-of-type(3n)]:before:bg-violet-200 [&>h2:nth-of-type(3n+2)]:before:bg-indigo-200 [&>h2]:mt-12 [&>h2]:flex [&>h2]:items-center [&>h2]:font-mono [&>h2]:text-sm [&>h2]:font-medium [&>h2]:leading-7 [&>h2]:text-slate-900 [&>h2]:before:mr-3 [&>h2]:before:h-3 [&>h2]:before:w-1.5 [&>h2]:before:rounded-r-full [&>h2]:before:bg-cyan-200 [&>ul]:mt-6 [&>ul]:list-['\2013\20'] [&>ul]:pl-5"#
                    ...
                    {
                        issue.as_ref().unwrap().crates
                            .iter()
                            .map(|crate_release|view!{
                                <CrateRelease
                    ...
                    <h2 class="mt-2 text-2xl font-bold text-slate-900">Pull Requests Merged this week</h2>
                    <ul role="list" class="space-y-6 mt-6">
                    {
                        merged_pull_requests.expect("")
                            .iter()
                            .map(|pull_request| view!{
                                <ActivityListItem
                    ...
                    //   }
                    >
                        <p>Here you can find two types of potential contribution: Pull Requests that might need review and Issues that might need to be worked on.</p>
                    </CalloutInfo>
                    <h2 class="mt-6 text-2xl font-bold text-slate-900">Pull Requests Opened this week</h2>
                    <ul role="list" class="space-y-6 mt-6">
                    {
                        opened_pull_requests.expect("")
                            .iter()
                            .map(|pull_request| view!{
                                <ActivityListItem
                    ...
                    <h2 class="mt-6 text-2xl font-bold text-slate-900">Issues Opened this week</h2>
                    <ul role="list" class="space-y-6 mt-6">
                    {
                        opened_issues.expect("")
                            .iter()
                            .map(|issue| view!{
                                <ActivityListItem
    ...
    view! {
      <li class="relative flex gap-x-4">
        ...
        <div class="relative flex h-6 w-6 flex-none items-center justify-center bg-white">
            {
                match icon {
                    ActivityListIcon::Default => view!{<><div class="h-1.5 w-1.5 rounded-full bg-gray-100 ring-1 ring-gray-300"></div></>},
                    ActivityListIcon::Check => view!{<>
        ...
        <p class="flex-auto py-0.5 text-xs leading-5 text-gray-500">
            <a href=url class="font-medium text-gray-900">{title}</a>
            " authored by "
            <span class="font-medium text-gray-900">{author}</span>
        </p>
        <time datetime=&date class="flex-none py-0.5 text-xs leading-5 text-gray-500">{date}</time>
    </li>
    }
}
#[component]
fn ActivityListComment(
    ...
    view! {
      <li class="relative flex gap-x-4">
        ...
        <img src="/this-week-in-bevydark" alt="" class="relative mt-3 h-6 w-6 flex-none rounded-full bg-gray-50"/>
        <div class="flex-auto rounded-md p-3 ring-1 ring-inset ring-gray-200">
          <div class="flex justify-between gap-x-4">
            <div class="py-0.5 text-xs leading-5 text-gray-500"><span class="font-medium text-gray-900">{author}</span> commented</div>
            <time datetime=time class="flex-none py-0.5 text-xs leading-5 text-gray-500">3d ago</time>
          </div>
    ...
    fn icon_svg(&self) -> impl IntoView {
        match self {
            CalloutType::Info => view! {
                <svg class=format!("h-5 w-5 {}", self.icon()) viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
    ...
    view! {
        <div class=format!("rounded-md p-4 {}", r#type.bg())>
            <div class="flex">
                ...
                <div class="ml-3">
                {
                    title.is_empty().not().then_some(
                        view!{
                            <h3 class=format!("text-sm font-medium {}", r#type.title())>{title}</h3>
                        }
                    )
                }
                <div class={
                    if link.is_none() {
                        ...
                        format!("ml-3 flex-1 md:flex md:justify-between {} {}", r#type.content(), r#type.child_links())
                    }
                }>
                    {children()}
                    {
                        link.map(|CalloutLink {href, label}| view!{
...
#[component]
fn Divider(#[prop(into)] title: String) -> impl IntoView {
    view! {
      <div class="relative mt-12 mb-6">
        ...
        <div class="relative flex justify-center">
          <span class="bg-white px-3 text-base font-semibold leading-6 text-gray-900">{title}</span>
        </div>
      </div>
    }
}
